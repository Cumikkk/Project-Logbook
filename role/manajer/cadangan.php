...
if (isset($_GET['pulihkan'])) {
    ...
    $cadangan = mysqli_fetch_assoc(mysqli_query($conn, "
        SELECT * FROM cadangan WHERE id=$id AND manajer_id=$manajer_id
    "));
    ...
    if ($cadangan) {
        ...
        if (mysqli_num_rows($cek) > 0) {
            ...
            header("Location: " . BASE_URL . "role/manajer/cadangan.php?pulihkan=success");
            exit;
        }
    } else {
        $error = "Data cadangan tidak ditemukan atau bukan milik divisi Anda.";
    }
}
...
// AMBIL DATA CADANGAN (hanya divisi milik manajer ini)
$data = mysqli_query($conn, "
    SELECT c.*, d.nama_divisi
    FROM cadangan c
    LEFT JOIN divisi d ON c.divisi_id = d.id
    WHERE c.manajer_id = $manajer_id
                ...
                <tbody id="tabelBody">
                    ...
                    ?>
                        <tr
                            ...
                            <!-- AKSI -->
                            <td>
                                <button class="btn btn-sm btn-success rounded-3 fw-semibold"
